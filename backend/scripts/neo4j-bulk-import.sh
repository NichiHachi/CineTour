...
# Ajouter directors si le fichier existe
if [[ -f "$CSV_DIR/directors.csv" ]]; then
    IMPORT_CMD="$IMPORT_CMD --nodes=Director=/var/lib/neo4j/import/$(basename $CSV_DIR)/directors.csv"
fi
...
if [[ -f "$CSV_DIR/directed_by.csv" ]]; then
    IMPORT_CMD="$IMPORT_CMD --relationships=DIRECTED_BY=/var/lib/neo4j/import/$(basename $CSV_DIR)/directed_by.csv"
fi
...
echo -e "${BLUE}5️⃣  Démarrage de Neo4j...${NC}"
docker exec $CONTAINER_NAME bash -c "neo4j start" || docker start $CONTAINER_NAME
...
echo -e "${BLUE}6️⃣  Création des index pour les performances...${NC}"
...
# Statistiques
echo ""
echo -e "${BLUE}7️⃣  Statistiques de la base:${NC}"
docker exec $CONTAINER_NAME cypher-shell -u neo4j -p cinetour_neo4j_password << 'EOF'
