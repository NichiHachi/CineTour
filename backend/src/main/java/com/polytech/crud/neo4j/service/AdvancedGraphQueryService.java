...
public class AdvancedGraphQueryService {
         ...
         * Intègre un coefficient de popularité (note * nombre de votes) pour
         * privilégier les films connus
         */
        public List<Map<String, Object>> findMoviesWithCommonActors(String idImdb, int minCommonActors) {
                String cypher = """
                                ...
                                WITH m2, COUNT(DISTINCT p) as commonActors, r
                                WHERE commonActors >= $minCommonActors
                                WITH m2, commonActors, r,
                                     CASE
                                         WHEN r.averageRating IS NOT NULL AND r.numVotes IS NOT NULL
                                         THEN commonActors * (r.averageRating * r.numVotes / 1000.0)
                                         ELSE toFloat(commonActors)
                                     END as popularityScore
                                RETURN m2.title as title,
                                       m2.idImdb as idImdb,
                                       m2.releaseYear as year,
                                       commonActors,
