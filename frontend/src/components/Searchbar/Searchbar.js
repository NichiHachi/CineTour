...
const Searchbar = () => {
    ...
    const [searchQuery, setSearchQuery] = useState(''); // Nouveau state
    ...
    const handleFilter = async (event) => {
        ...
        setSearchQuery(searchWord); // Sauvegarder la recherche
    ...
    // Nouvelle fonction pour rediriger vers la page de résultats
    const handleSearchSubmit = () => {
        if (searchQuery.trim()) {
            navigate(`/search-results?q=${encodeURIComponent(searchQuery)}`);
            setFilteredData([]); // Fermer les suggestions
        }
    }
            ...
            <div className="search-section">
                <div className="search-input" ref={searchRef}>
                    <input
                        ...
                        onKeyPress={handleKeyPress} // Ajout du gestionnaire
                    />
                </div>
